...
class TwistTeleopNode(Node):
    def __init__(self):
        ...
        self.last_gripper_state = None  # Tracks if we already sent the open/close command
        ...
        self.get_logger().info('  LEFT HAND GRIPPER CONTROL:')
    ...
    def _timer_cb(self):
        ...
        # Process frame ( left, and right hand completely)
        ...
        # ---------------------------------------------------------
        # GRIPPER CONTROL (Left Hand gestures, Right Hand clutch)
        # ---------------------------------------------------------
        # 1. Only allow gripper control if the Right Hand clutch is engaged (closed fist)
        if right.detected and right.fist_closed:
            
            # 2. Check the Left Hand to control the gripper
            if left.detected:
                # If left hand is a full fist OR pinching -> Close Gripper
                # If left hand is open -> Open Gripper
                should_close = left.fist_closed or left.gripper_closed
                
                # 3. Only send the action command if the state CHANGED
        ...
        try:
            ...
            self.get_logger().info(f"EE Pos -> X:{self.end_effector_x:.3f} Y:{self.end_effector_y:.3f} Z:{self.end_effector_z:.3f}")
   ...
   # gripper closer function missing
   
    def _send_gripper_command(self, close=True):
            """Sends an async goal to the Robotiq Gripper Action Server."""
            if not self.gripper_client.wait_for_server(timeout_sec=0.1):
                self.get_logger().warn('Gripper action server not ready')
                return

            goal = GripperCommand.Goal()
            # For Robotiq: 0.0 usually means open, 0.8 means fully closed ,only position required
            """ Closed:
                position: 0.4867
                effort: nan
                ...
                position: 0.0713
                effort: nan"""
            # Gap size in meters: 0.4867 is closed, 0.0713 is open

            goal.command.position = 0.4867 if close else 0.0713
            # goal.command.max_effort = 0.0 # gripper at 100 N
                #Set it to 0.0. When you send 0.0, the Kinova driver is smart enough to just use the default, 
                #safe closing force configured in the hardware.
            self.get_logger().info(f'Sending Gripper Goal: {"CLOSE" if close else "OPEN"}')
            self.gripper_client.send_goal_async(goal)

 # 0.0 (open )
# 0.8 (closed)
#If your driver is configured for meters, sending 0.8 tells the gripper to open to 800 millimeters, which triggers the "ABOVE MAXIMUM" error!
