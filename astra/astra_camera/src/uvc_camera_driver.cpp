...
UVCCameraDriver::UVCCameraDriver(rclcpp::Node *node,
                                 std::shared_ptr<Parameters> parameters,
                                 sensor_msgs::msg::CameraInfo camera_info,
                                 const std::string &serial_number)
    : node_(node), logger_(node_->get_logger()),
      parameters_(std::move(parameters)), camera_info_(std::move(camera_info)) {
  ...
  setAndGetNodeParameter<std::string>(parameters_, config_.format,
                                      "uvc_camera_format", "mjpeg");
  setAndGetNodeParameter(parameters_, config_.retry_count, "uvc_retry_count",
                         500);
  ...
  setAndGetNodeParameter(parameters_, camera_name_, "camera_name",
                         camera_name_);
  setAndGetNodeParameter<std::string>(parameters_, config_.frame_id,
  ...
  setAndGetNodeParameter<std::string>(parameters_, rgb_qos_profile_str,
                                      "rgb_qos_profile", "default");
  setAndGetNodeParameter<std::string>(parameters_, rgb_info_qos_profile_str,
                                      "rgb_info_qos_profile", "default");
  color_qos_profile_ = getRMWQosProfileFromString(rgb_qos_profile_str);
  color_info_qos_profile_ =
      getRMWQosProfileFromString(rgb_info_qos_profile_str);
  ...
  image_publisher_ = node_->create_publisher<sensor_msgs::msg::Image>(
      "color/image_raw",
      rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(color_qos_profile_),
                  color_qos_profile_));
  camera_info_publisher_ =
      node_->create_publisher<sensor_msgs::msg::CameraInfo>(
          "color/camera_info", rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(
                                               color_info_qos_profile_),
                                           color_info_qos_profile_));
...
void UVCCameraDriver::openCamera() {
  ...
  if (err != UVC_SUCCESS) {
    ...
    RCLCPP_ERROR_STREAM(logger_, "find uvc device failed, retry "
...
void UVCCameraDriver::setVideoMode() {
  ...
  RCLCPP_INFO_STREAM(logger_, "set uvc mode "
                                  << width << "x" << height << "@" << fps
                                  << " format "
...
void UVCCameraDriver::startStreaming() {
  ...
  if (stream_err != UVC_SUCCESS) {
    RCLCPP_ERROR_STREAM(logger_, "uvc start streaming error "
...
bool UVCCameraDriver::getUVCExposureCb(
...
bool UVCCameraDriver::setUVCExposureCb(
...
bool UVCCameraDriver::getUVCGainCb(
    const std::shared_ptr<GetInt32::Request> &request,
    std::shared_ptr<GetInt32::Response> &response) {
  (void)request;
  uint16_t data;
...
bool UVCCameraDriver::setUVCGainCb(
    const std::shared_ptr<SetInt32::Request> &request,
    std::shared_ptr<SetInt32::Response> &response) {
...
bool UVCCameraDriver::getUVCWhiteBalanceCb(
    const std::shared_ptr<GetInt32::Request> &request,
    std::shared_ptr<GetInt32::Response> &response) {
  (void)request;
  uint16_t data;
...
bool UVCCameraDriver::setUVCWhiteBalanceCb(
    const std::shared_ptr<SetInt32::Request> &request,
    std::shared_ptr<SetInt32::Response> &response) {
  if (request->data == 0) {
...
bool UVCCameraDriver::setUVCAutoWhiteBalanceCb(
    ...
    std::shared_ptr<SetBool::Response> &response) {
  auto err =
      uvc_set_white_balance_temperature_auto(device_handle_, request->data);
  if (err != UVC_SUCCESS) {
    auto msg = uvc_strerror(err);
    RCLCPP_ERROR_STREAM(logger_, "setUVCMirrorCb " << msg);
    response->success = false;
...
bool UVCCameraDriver::getUVCMirrorCb(
    const std::shared_ptr<GetInt32::Request> &request,
    std::shared_ptr<GetInt32::Response> &response) {
  (void)request;
  int16_t roll;
...
bool UVCCameraDriver::setUVCMirrorCb(
    const std::shared_ptr<SetBool::Request> &request,
    std::shared_ptr<SetBool::Response> &response) {
  int16_t roll;
  if (request->data) {
...
bool UVCCameraDriver::toggleUVCCamera(
    const std::shared_ptr<SetBool::Request> &request,
    std::shared_ptr<SetBool::Response> &response) {
  (void)response;
  if (request->data) {
...
int UVCCameraDriver::UVCGetControl(int control, int unit, int len,
  ...
  if (ret < 0) {
    ...
    RCLCPP_ERROR(logger_, "Failed to get control %d on unit %d: %s", control,
                 unit, uvc_strerror(err));
    return -1;
  }
