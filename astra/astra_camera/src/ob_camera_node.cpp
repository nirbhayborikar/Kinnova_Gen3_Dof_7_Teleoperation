...
void OBCameraNode::init() {
  ...
  poll_stream_thread_ =
      std::make_shared<std::thread>([this]() { pollFrame(); });
  if (enable_point_cloud_) {
    point_cloud_processor_ =
        std::make_unique<PointCloudXyzNode>(node_, parameters_);
  }
  if (enable_colored_point_cloud_) {
    colored_point_cloud_processor_ =
        std::make_unique<PointCloudXyzrgbNode>(node_, parameters_);
  }
...
OBCameraNode::OBCameraNode(rclcpp::Node *node,
                           ...
                           bool use_uvc_camera)
    : node_(node), device_(std::move(device)),
      parameters_(std::move(parameters)), logger_(node->get_logger()),
...
void OBCameraNode::pollFrame() {
  ...
  while (rclcpp::ok() && run_streaming_poller_ && is_running_) {
    ...
    auto has_stream_started =
        stream_started_cv_.wait_for(lock, 1000ms, [this]() {
          return std::any_of(IMAGE_STREAMS.begin(), IMAGE_STREAMS.end(),
                             [this](const auto &stream_index) {
                               return stream_started_[stream_index];
                             });
        });
    ...
    if (status != openni::STATUS_OK) {
      RCLCPP_ERROR_STREAM(
          logger_, "read " << stream_name_[stream_index] << " stream failed "
  ...
  for (const auto &stream_index : IMAGE_STREAMS) {
    if (stream_started_[stream_index]) {
      ...
      RCLCPP_INFO_STREAM(logger_,
                         "Stopped stream " << stream_name_[stream_index]);
...
void OBCameraNode::setupDevices() {
  for (const auto &stream_index : IMAGE_STREAMS) {
    ...
    if (enable_stream_[stream_index] &&
      ...
      if (status != openni::STATUS_OK) {
        std::stringstream ss;
        ss << "Couldn't create depth video stream: "
           << openni::OpenNI::getExtendedError();
...
void OBCameraNode::setupVideoMode() {
  if (!use_uvc_camera_ && enable_stream_[INFRA1] && enable_stream_[COLOR]) {
    RCLCPP_WARN_STREAM(logger_, "Infrared and Color streams are enabled. "
    ...
    if (device_->hasSensor(stream_index.first) &&
      ...
      if (!is_supported_mode) {
        ...
        if (is_default_mode_supported) {
          RCLCPP_WARN_STREAM(logger_, "Default video mode "
                                          << default_video_mode
                                          << " is supported. "
      ...
      if (is_supported_mode) {
        RCLCPP_INFO_STREAM(logger_, "set " << stream_name_[stream_index]
...
void OBCameraNode::setupUVCCamera() {
  if (use_uvc_camera_) {
    ...
    uvc_camera_driver_ = std::make_shared<UVCCameraDriver>(
        node_, parameters_, color_camera_info, serial_number);
  } else {
...
void OBCameraNode::getParameters() {
  setAndGetNodeParameter(parameters_, camera_name_, "camera_name",
                         camera_name_);
  ...
  for (const auto &stream_index : IMAGE_STREAMS) {
    ...
    setAndGetNodeParameter(parameters_, width_[stream_index], param_name,
                           IMAGE_WIDTH);
    param_name = stream_name_[stream_index] + "_height";
    setAndGetNodeParameter(parameters_, height_[stream_index], param_name,
                           IMAGE_HEIGHT);
    param_name = stream_name_[stream_index] + "_fps";
    setAndGetNodeParameter(parameters_, fps_[stream_index], param_name,
                           IMAGE_FPS);
    param_name = "enable_" + stream_name_[stream_index];
    setAndGetNodeParameter(parameters_, enable_stream_[stream_index],
                           param_name, false);
    ...
    setAndGetNodeParameter(parameters_, optical_frame_id_[stream_index],
                           param_name, optical_frame_id_[stream_index]);
    param_name = stream_name_[stream_index] + "_qos";
    setAndGetNodeParameter<std::string>(parameters_, image_qos_[stream_index],
                                        param_name, "default");
    param_name = stream_name_[stream_index] + "_camera_info_qos";
    setAndGetNodeParameter<std::string>(
        parameters_, camera_info_qos_[stream_index], param_name, "default");
  }
  for (const auto &stream_index : IMAGE_STREAMS) {
  ...
  setAndGetNodeParameter(parameters_, tf_publish_rate_, "tf_publish_rate",
                         10.0);
  setAndGetNodeParameter(parameters_, camera_link_frame_id_,
                         "camera_link_frame_id", DEFAULT_BASE_FRAME_ID);
  setAndGetNodeParameter(parameters_, depth_registration_, "depth_registration",
                         false);
  setAndGetNodeParameter(parameters_, color_depth_synchronization_,
  ...
  setAndGetNodeParameter(parameters_, color_roi_.height, "color_roi_height",
                         -1);
  ...
  setAndGetNodeParameter(parameters_, depth_roi_.height, "depth_roi_height",
                         -1);
  setAndGetNodeParameter(parameters_, depth_scale_, "depth_scale", 1);
  setAndGetNodeParameter(parameters_, enable_point_cloud_, "enable_point_cloud",
                         true);
  setAndGetNodeParameter(parameters_, enable_colored_point_cloud_,
                         "enable_colored_point_cloud", false);
  setAndGetNodeParameter<std::string>(parameters_, point_cloud_qos_,
                                      "point_cloud_qos", "default");
  setAndGetNodeParameter(parameters_, enable_publish_extrinsic_,
                         "enable_publish_extrinsic", false);
  setAndGetNodeParameter<std::string>(parameters_, ir_info_url_, "ir_info_url",
                                      "");
  setAndGetNodeParameter<std::string>(parameters_, color_info_url_,
                                      "color_info_url", "");
  if (enable_colored_point_cloud_) {
    ...
    if (enable_stream_[stream_index]) {
      ...
      image_publishers_[stream_index] =
          node_->create_publisher<sensor_msgs::msg::Image>(
              topic, rclcpp::QoS(
                         rclcpp::QoSInitialization::from_rmw(image_qos_profile),
                         image_qos_profile));
      topic = name + "/camera_info";
      auto camera_info_qos = camera_info_qos_[stream_index];
      auto camera_info_qos_profile =
          getRMWQosProfileFromString(camera_info_qos);
      camera_info_publishers_[stream_index] =
          node_->create_publisher<CameraInfo>(
              topic, rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(
                                     camera_info_qos_profile),
                                 camera_info_qos_profile));
    }
  }
  if (enable_publish_extrinsic_) {
    extrinsics_publisher_ = node_->create_publisher<Extrinsics>(
        "extrinsic/depth_to_color", rclcpp::QoS{1}.transient_local());
  }
}

void OBCameraNode::publishStaticTF(const rclcpp::Time &t,
...
void OBCameraNode::calcAndPublishStaticTransform() {
  ...
  if (std::isnan(transition[0]) || std::isnan(transition[1]) ||
  ...
  publishStaticTF(tf_timestamp, trans, Q, camera_link_frame_id_,
                  frame_id_[COLOR]);
  ...
  publishStaticTF(tf_timestamp, zero_trans, zero_rot, camera_link_frame_id_,
                  frame_id_[DEPTH]);
  auto ex_msg =
      obExtrinsicsToMsg(rotation, transition, "depth_to_color_extrinsics");
...
void OBCameraNode::publishDynamicTransforms() {
  RCLCPP_WARN(logger_, "Publishing dynamic camera transforms (/tf) at %g Hz",
              tf_publish_rate_);
  static std::mutex mu;
...
void OBCameraNode::publishStaticTransforms() {
  ...
  if (tf_publish_rate_ > 0) {
    tf_thread_ =
        std::make_shared<std::thread>([this]() { publishDynamicTransforms(); });
  } else {
...
void OBCameraNode::setImageRegistrationMode(bool enable) {
  if (!device_->isImageRegistrationModeSupported(
    ...
    RCLCPP_WARN_STREAM(logger_,
                       "Current do not support " << magic_enum::enum_name(
                           openni::IMAGE_REGISTRATION_DEPTH_TO_COLOR));
    return;
  }
  auto mode = enable ? openni::IMAGE_REGISTRATION_DEPTH_TO_COLOR
...
void OBCameraNode::onNewFrameCallback(const openni::VideoFrameRef &frame,
  ...
  if (stream_index == DEPTH) {
    cv::resize(image, scaled_image,
               cv::Size(width * depth_scale_, height * depth_scale_), 0, 0,
               cv::INTER_NEAREST);
  }
  auto image_msg =
      cv_bridge::CvImage(std_msgs::msg::Header(), encoding_.at(stream_index),
                         stream_index == DEPTH ? scaled_image : image)
          .toImageMsg();
  CHECK_NOTNULL(image_msg);
  auto timestamp = node_->now();
  image_msg->header.stamp = timestamp;
  image_msg->header.frame_id = depth_registration_
  ...
  camera_info.header.frame_id = depth_registration_
                                    ? depth_aligned_frame_id_[stream_index]
                                    : optical_frame_id_[stream_index];
  ...
  if (width * 9 == height * 16) {
    ...
    if (status != openni::STATUS_OK) {
      RCLCPP_ERROR_STREAM(logger_,
                          "setProperty XN_MODULE_PROPERTY_D2C_RESOLUTION "
                              << openni::OpenNI::getExtendedError());
    }
